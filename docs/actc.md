...
- core REAL functions: a local no-argument `REAL FUNC` may return any directly
  ...
  `FMin(A,B)` and `FMax(A,B)` with named REAL operands in assignments, REAL
  printing, and conditions. Those calls use independently selected helpers with
  complete MATH1 NaN/signed-zero selection semantics. Neither path is general
  REAL expression, control-flow, nested-call, or return lowering
