...
- core REAL functions: a local no-argument `REAL FUNC` may return any directly
  ...
  `FSign(A)`, `FTrunc(A)`, `FFloor(A)`, `FCeil(A)`, `FRound(A)`, `FFrac(A)`, `FMod(A,B)`, `FHypot(A,B)`, `FPow(A,B)`, `FExp(A)`, `FLn(A)`, `FLog2(A)`, `FLog10(A)`, `FSin(A)`, `FCos(A)`, `FTan(A)`, `FATan(A)`, `FMin(A,B)`, and `FMax(A,B)` with named REAL operands in
