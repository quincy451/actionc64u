...
- `tools/build_actc_overlay_emit_native_real_postfix_object.sh` builds
  ...
  6,091-byte image leaves 2,101 bytes free in the 8 KiB execution window.
- `tools/build_actc_overlay_emit_native_real_postfix_control_object.sh` builds
  ...
  returns, and recursive/reentrant functions remain unsupported. Its 6,960-byte
  image leaves 1,232 bytes free under a dedicated 1 KiB capacity gate, while
...
- `tools/build_actc_overlay_emit_native_real_postfix_multi_control_object.sh`
  ...
  and `__reNN` exports keep each long branch fully relocatable. The 7,082-byte
  image leaves 1,110 bytes free under a dedicated 1 KiB gate; passes L and M
...
- `tools/build_actc_overlay_emit_native_real_postfix_extended_control_object.sh`
  ...
  occupied slot. Its 7,085-byte image leaves 1,107 bytes free under the 1 KiB
...
- `tools/build_actc_overlay_emit_native_real_postfix_early_return_object.sh`
  ...
  7,109-byte image leaves 1,083 bytes free under the 1 KiB gate; passes L
...
- `tools/build_actc_overlay_emit_native_real_postfix_loop_object.sh` builds
  ...
  relocatable `__rbNN` back-edge plus `__rzNN` while-exit labels. Its 7,113-byte
  image leaves 1,079 bytes free under the 1 KiB gate; passes L through P retain
...
- `tools/build_actc_overlay_emit_native_real_postfix_loop_exit_object.sh`
  ...
  7,296-byte image leaves 896 bytes free under a dedicated 768-byte gate;
...
- `tools/build_actc_overlay_emit_native_real_postfix_for_object.sh` builds
  ...
  prevent wraparound from restarting the loop. Its 7,790-byte image leaves 402
...
- `tools/build_actc_overlay_emit_native_real_postfix_for_dynamic_object.sh`
  ...
  fixture exercises both forms and produces 7.0 twice. Its 8,109-byte image
  leaves 83 bytes free under a dedicated 32-byte gate; pass S retains its
...
- `tools/build_actc_overlay_emit_native_real_postfix_literal_object.sh` builds
  ...
  plus ordinary `RT_F_DIV`/`RT_F_MUL` imports. The 7,439-byte image leaves 753
