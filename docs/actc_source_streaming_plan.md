...
Its 6,084-byte image leaves 2,108 bytes in `$A000-$BFFF`.
...
Its 6,953-byte image leaves 1,239 bytes under a 1 KiB reserve without consuming
...
with a distinct false/end export pair per control. Its 7,075-byte image leaves
1,117 bytes under the same 1 KiB reserve while passes L and M retain their
...
7,078-byte image leaves 1,114 bytes under the 1 KiB reserve while passes L, M,
...
true, early else, and fallback paths. The 7,102-byte image leaves 1,090 bytes
...
and 3.0 in native VICE and the Idun generated-6502 path. The 7,106-byte image
leaves 1,086 bytes under the 1 KiB reserve while passes L through P retain
...
generated-6502 path. The 7,289-byte image leaves 903 bytes under a dedicated
...
and the Idun generated-6502 path. The 7,783-byte image leaves 409 bytes under a
...
generated-6502 path both produce 7.0 twice. The 8,102-byte image leaves 90
...
execution both produce `-1`, `0`, and `1`. The angle fixture continues to
	materialize pi as `DB 0F 49 40`. The 7,432-byte image leaves 760 bytes free
