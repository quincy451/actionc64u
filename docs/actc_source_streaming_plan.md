...
Its 6,120-byte image leaves 2,072 bytes in `$A000-$BFFF`.
...
Its 6,989-byte image leaves 1,203 bytes under a 1 KiB reserve without consuming
...
with a distinct false/end export pair per control. Its 7,111-byte image leaves
1,081 bytes under the same 1 KiB reserve while passes L and M retain their
...
7,114-byte image leaves 1,078 bytes under the 1 KiB reserve while passes L, M,
...
true, early else, and fallback paths. The 7,138-byte image leaves 1,054 bytes
...
and 3.0 in native VICE and the Idun generated-6502 path. The 7,142-byte image
leaves 1,050 bytes under the 1 KiB reserve while passes L through P retain
...
generated-6502 path. The 7,325-byte image leaves 867 bytes under a dedicated
...
and the Idun generated-6502 path. The 7,819-byte image leaves 373 bytes under a
...
generated-6502 path both produce 7.0 twice. The 8,138-byte image leaves 54
...
materialize pi as `DB 0F 49 40`. The 7,468-byte image leaves 724 bytes free
