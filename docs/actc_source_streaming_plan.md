...
Its 6,098-byte image leaves 2,094 bytes in `$A000-$BFFF`.
...
Its 6,967-byte image leaves 1,225 bytes under a 1 KiB reserve without consuming
...
with a distinct false/end export pair per control. Its 7,089-byte image leaves
1,103 bytes under the same 1 KiB reserve while passes L and M retain their
...
7,092-byte image leaves 1,100 bytes under the 1 KiB reserve while passes L, M,
...
true, early else, and fallback paths. The 7,116-byte image leaves 1,076 bytes
...
and 3.0 in native VICE and the Idun generated-6502 path. The 7,120-byte image
leaves 1,072 bytes under the 1 KiB reserve while passes L through P retain
...
generated-6502 path. The 7,303-byte image leaves 889 bytes under a dedicated
...
and the Idun generated-6502 path. The 7,797-byte image leaves 395 bytes under a
...
generated-6502 path both produce 7.0 twice. The 8,116-byte image leaves 76
...
execution both produce `-1`, `0`, and `1`. The angle fixture continues to
	materialize pi as `DB 0F 49 40`. The 7,446-byte image leaves 746 bytes free
