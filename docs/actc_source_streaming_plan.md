...
Its 6,091-byte image leaves 2,101 bytes in `$A000-$BFFF`.
...
Its 6,960-byte image leaves 1,232 bytes under a 1 KiB reserve without consuming
...
with a distinct false/end export pair per control. Its 7,082-byte image leaves
1,110 bytes under the same 1 KiB reserve while passes L and M retain their
...
7,085-byte image leaves 1,107 bytes under the 1 KiB reserve while passes L, M,
...
true, early else, and fallback paths. The 7,109-byte image leaves 1,083 bytes
...
and 3.0 in native VICE and the Idun generated-6502 path. The 7,113-byte image
leaves 1,079 bytes under the 1 KiB reserve while passes L through P retain
...
generated-6502 path. The 7,296-byte image leaves 896 bytes under a dedicated
...
and the Idun generated-6502 path. The 7,790-byte image leaves 402 bytes under a
...
generated-6502 path both produce 7.0 twice. The 8,109-byte image leaves 83
...
execution both produce `-1`, `0`, and `1`. The angle fixture continues to
	materialize pi as `DB 0F 49 40`. The 7,439-byte image leaves 753 bytes free
