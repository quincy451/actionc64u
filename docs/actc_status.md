...
- `ACTC_OVL9.BIN` is 7,599 bytes after word-function call/return lowering,
  ...
  128-byte minimum reserve. `ACTC_OVL6.BIN` is 8,052 bytes with 140 bytes free
  under a 96-byte gate. `ACTC_OVL7.BIN` is 6,999 bytes with 1,193 bytes free in
  ...
  is 6,120 bytes with 2,072 bytes free. Function-control pass M is 6,989 bytes
  with 1,203 bytes free under its dedicated 1 KiB gate. Two-control pass N is
  7,111 bytes with 1,081 bytes free under the same gate. Four-control pass O is
  7,114 bytes with 1,078 bytes free. Conditional-early-return pass P is 7,138
  bytes with 1,054 bytes free. REAL-function loop pass Q is 7,142 bytes with
  1,050 bytes free. Plain-loop/nearest-EXIT pass R is 7,325 bytes with 867
  bytes free under its dedicated 768-byte gate. Constant-bound CARD-counter
  `FOR` pass S is 7,819 bytes with 373 bytes free under its dedicated 256-byte
  gate. Named-bound `FOR` pass T is 8,138 bytes with 54 bytes free under its
  dedicated 32-byte gate. Literal/control pass U is 7,468 bytes with 724 bytes
...
- The bounded REAL value parser also recognizes `FSign(A)`, `FTrunc(A)`, `FFloor(A)`, `FCeil(A)`, `FRound(A)`, `FFrac(A)`, `FMod(A,B)`, `FHypot(A,B)`, `FPow(A,B)`, `FExp(A)`, `FLn(A)`, `FLog2(A)`, `FLog10(A)`, `FSin(A)`, `FCos(A)`, `FTan(A)`, `FATan(A)`,
...
- Pass L consumes the bounded child-first REAL stream from passes 6 and 7 for
  ...
  runtime objects. Pass L is 6,120 bytes with 2,072 bytes free.
