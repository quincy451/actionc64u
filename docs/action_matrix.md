...
| Emit `.AVO` -> `.AVM.TXT` final-image skeleton | Yes | No | Partial | Yes | `ALINK.PRG` now loads deterministic `AVO1` object stubs from `OBJ/`, parses export/call/import/payload metadata, and writes `BIN/<NAME>.AVM.TXT` on the host fs tree as a compact text form of the final image: `entry <offset>` plus `db $..` payload bytes. The focused proof then host-packs that text through `avm_pack.py --text --flags 1` and verifies the exact `AVM1` bytes. This is closer to the final linker artifact, but it is still not direct on-target binary emission or a full dead-strip object merger. |
