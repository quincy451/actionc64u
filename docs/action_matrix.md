...
| Emit `.ACT` -> `.AVO` object stubs | Yes | No | Partial | Yes | `ACTC.PRG` now emits deterministic `AVO1` text object stubs into `OBJ/` for tracked project modules, including the current inferred runtime-import list from simple source-pattern scanning. The current UDOS-native slice is a project-aware front end/object emitter only, not a full compiler. |
