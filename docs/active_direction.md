...
Its bounded matcher captures initializer, argument, result, parameter-bind,
comparison, and return storage, so declaration and parameter names need not map
to one fixed slot order.
...
ABI.
Pass K now also proves one selected binary REAL return over two parameters. It
uses a hidden non-aliasing result cell and can select arithmetic, min/max,
...
products through ordinary OBJ1 relocations. Pass M now adds one nonnested
`IF`/`ELSE` per supported REAL function using `rt_f_cmp` and relocatable
internal code labels; its shared fixture executes both paths and prints `34`.
...
Bounded named-REAL `FSign`, `FTrunc`, `FFloor`, `FCeil`, `FRound`, `FFrac`, `FMod`, `FHypot`, `FPow`, `FExp`, `FLn`, `FLog2`, `FLog10`, `FSin`, `FCos`, `FTan`, `FATan`, `FATan2`, `FASin`, `FMin`, `FMax`, and `FClamp` calls now have
...
it currently enumerates 1377 shape probes.
...
1. Extend the combined Pass U REAL-literal/control path beyond comma-grouped
   uninitialized locals to the nested expressions and calls used by the
   remaining portable MATH1 routines, then package each routine as a
   link-selected OBJ module.
