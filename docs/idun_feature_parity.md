...
local call without creating a phantom library import. Pass 7 is now 6,801 bytes
with 1,391 bytes free in its 8 KiB window; native VICE verifies 5.0 in both the
...
inventories are 1,368 broad direct-PRG shapes, 196 non-runtime source-backed
object-emission shapes, and 302
compiled-runtime relocation-oracle cases. Pass L is 6,067 bytes with 2,125
bytes free; pass M is 6,936 bytes with
1,256 bytes free; pass N is 7,058 bytes with 1,134 bytes free under its 1 KiB
gate; pass O is 7,061 bytes with 1,131 bytes free under the same gate. The
pass-P image is 7,085 bytes with 1,107 bytes free under the same gate. The
pass-Q image is 7,089 bytes with 1,103 bytes free under the same gate. Pass R is
7,272 bytes with 920 bytes free under its dedicated 768-byte gate. Pass S is
7,766 bytes with 426 bytes free under its dedicated 256-byte gate. Pass T is
8,085 bytes with 107 bytes free under its dedicated 32-byte gate. Pass U is 7,415
bytes with 777 bytes free under its dedicated 640-byte gate. The public
MATH1 gap is now 24 routines.
