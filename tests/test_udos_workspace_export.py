...
class TestUdosWorkspaceExport(unittest.TestCase):
    ...
    def test_export_creates_udos_tree_with_guides_examples_and_libs(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue((lib_dir / "RT_F_DEG_TO_RAD.OBJ").is_file())
            self.assertTrue((lib_dir / "RT_F_RAD_TO_DEG.OBJ").is_file())
            ...
            self.assertIn("REAL FUNC DegToRad(REAL degrees)", math1_contents)
            self.assertIn("REAL FUNC RadToDeg(REAL radians)", math1_contents)
            ...
            f_deg_to_rad_contents = (
                lib_dir / "RT_F_DEG_TO_RAD.OBJ"
            ).read_text(encoding="ascii")
            self.assertIn("x rt_f_deg_to_rad 0 20", f_deg_to_rad_contents)
            self.assertIn("b u0M", f_deg_to_rad_contents)
            self.assertIn("u rt_f_mul", f_deg_to_rad_contents)
            self.assertIn("35 FA 8E 3C", f_deg_to_rad_contents)
            self.assertIn("n rt_f_deg_to_rad", f_deg_to_rad_contents)
            f_rad_to_deg_contents = (
                lib_dir / "RT_F_RAD_TO_DEG.OBJ"
            ).read_text(encoding="ascii")
            self.assertIn("x rt_f_rad_to_deg 0 20", f_rad_to_deg_contents)
            self.assertIn("b u0M", f_rad_to_deg_contents)
            self.assertIn("u rt_f_mul", f_rad_to_deg_contents)
            self.assertIn("E0 2E 65 42", f_rad_to_deg_contents)
            self.assertIn("n rt_f_rad_to_deg", f_rad_to_deg_contents)
